...
class Generator:
    ...
    def putDither(self, row, col, k):
        self.stats['positionsVisited'] += 1
        startX, startY, endX, endY = self.getSliceBounds(row, col)
        targetSlice = self.ditherImg[startY:endY, startX:endX]
        # Brighten the target to match maximum typable in first overtype
        targetSlice = gammaCorrect(targetSlice, self.maxGamma)
        # Get ID of best match
        if k == 1:
        ...
        if bestMatch:
            ...
            self.comboGrid.put(row, col, bestMatch)
            # Apply dither
            self.applyDither(row, col, targetSlice)
            self.mockupImg[startY:endY, startX:endX] = self.compositeAdj(row, col)
            return True
        else:
            # print("already good")
    ...
    def applyDither(self, row, col, targetImgSlice, amount=0.5):
        # Calculate error between chosen combo and target subslice
        startX, startY, endX, endY = self.getSliceBounds(row, col)
        mockupSlice = self.mockupImg[startY:endY, startX:endX]
        error = np.average(targetImgSlice) - np.average(mockupSlice)
        error *= 0.5
        # print(error)
        # self.ditherImg[startY:endY, startX:endX] = cv2.add(
        #         self.targetImg[startY:endY, startX:endX], error)
        # Dither Right
        if (col + 1 < self.cols):
            startX, startY, endX, endY = self.getSliceBoundsDither(row, col+1)
            self.ditherImg[startY:endY, startX:endX] = cv2.add(
                self.ditherImg[startY:endY, startX:endX], error * 7/16)
        # Dither Bottom Left
        if (row + 1 < self.rows and col - 1 >= 0):
            startX, startY, endX, endY = self.getSliceBoundsDither(row+1, col-1)
            self.ditherImg[startY:endY, startX:endX] = cv2.add(
                self.ditherImg[startY:endY, startX:endX], error * 3/16)
        # Dither Bottom
        if (row + 1 < self.rows):
            startX, startY, endX, endY = self.getSliceBoundsDither(row+1, col)
            self.ditherImg[startY:endY, startX:endX] = cv2.add(
                self.ditherImg[startY:endY, startX:endX], error * 5/16)
        # Dither BottomRight
        if (row + 1 < self.rows and col + 1 < self.cols):
            startX, startY, endX, endY = self.getSliceBoundsDither(row+1, col+1)
            self.ditherImg[startY:endY, startX:endX] = cv2.add(
                self.ditherImg[startY:endY, startX:endX], error * 1/16)
        # Set this slice back to original
        # startX, startY, endX, endY = self.getSliceBounds(row, col)
        # self.ditherImg[startY:endY, startX:endX] = self.targetImg[startY:endY, startX:endX]
    ...
    def generateLayers(self, compareModes=['m'], numAdjustPasses=0,
        ...
        self.positions = ditherPositions()
